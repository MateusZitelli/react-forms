...
describe('<Input />:', function() {
  ...
  it('renders into <input type="text" /> by default', function() {
    ...
    var element = React.createElement(Input, {value: value, className: "someClass"});
  ...
  it('renders into component provided via props', function() {
    ...
    var element = (
      React.createElement(Input, {
        value: value.get('a'), 
        className: "someClass", 
        input: React.createElement("input", {type: "password"})}
        )
    );
    var component = TestUtils.renderIntoDocument(element);
    var inputs = TestUtils.scryRenderedDOMComponentsWithTag(component, 'input');
  ...
  it('renders into component provided via schema node', function() {
    var schema = Mapping({a: Scalar({input: React.createElement("input", {type: "password"})})});
    var value = Value.create(schema);
    var element = React.createElement(Input, {value: value.get('a'), className: "someClass"});
    var component = TestUtils.renderIntoDocument(element);
    var inputs = TestUtils.scryRenderedDOMComponentsWithTag(component, 'input');
    assert.equal(inputs.length, 1);
    var input = inputs[0];
  ...
  it('renders "name" attribute to the <input />', function() {
    ...
    component = TestUtils.renderIntoDocument(React.createElement(Input, {value: value.get('a')}));
    input = TestUtils.findRenderedDOMComponentWithTag(component, 'input');
    assert.equal(input.getDOMNode().name, 'a');

    component = TestUtils.renderIntoDocument(React.createElement(Input, {value: value.get('b').get('c')}));
    input = TestUtils.findRenderedDOMComponentWithTag(component, 'input');
    assert.equal(input.getDOMNode().name, 'b__c');

    component = TestUtils.renderIntoDocument(React.createElement(Input, {value: value.get('d').get(1)}));
    input = TestUtils.findRenderedDOMComponentWithTag(component, 'input');
    assert.equal(input.getDOMNode().name, 'd__1');

    component = TestUtils.renderIntoDocument(React.createElement(Input, {value: value.get('e')}));
    input = TestUtils.findRenderedDOMComponentWithTag(component, 'input');

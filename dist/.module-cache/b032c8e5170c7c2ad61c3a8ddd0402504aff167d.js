...
describe('forms', function()  {
  ...
  describe('RadioButtonGroup', function() {
      ...
      function createAndMount(props) {
          var component = React.createElement(RadioButtonGroup, React.__spread({},  props));
      ...
      function getButtonsDOM(component) {
          return TestUtils.scryRenderedDOMComponentsWithClass(
              ...
              .map(function(c)  {return c.getDOMNode();});
      }

      function assertSelected(buttons, value) {
          assert.equal(
              buttons.filter(function(b)  {return b.checked && b.value === value;}).length,
              1);
      }

      function assertNoSelected(buttons) {
          assert.equal(buttons.filter(function(b)  {return b.checked;}).length, 0);
      }
      ...
      it('triggers onChange when user clicks on value', function() {
          ...
          var component = createAndMount({options: sexOptions, onChange:onChange});
